...
public class ScrollableCounter: UIView {
  ...
  init(items: [UIView], frame: CGRect = CGRect.zero, gradientColor: UIColor? = nil, gradientStop: Float? = nil) {
    assert(!items.isEmpty, "ScrollableCounter must be initialized with non empty array of items.")
    for item in items {
   ...
   */
  private func buildAnimations(direction: ScrollDirection, duration: TimeInterval, index: Int) -> UIViewPropertyAnimator {
  ...
  private func calculateDirection(toIndex index: Int) -> ScrollDirection {
    ...
    if currentIndex == index {
      if currentItem.frame.origin.y > 0 {
        direction = .up
      } else {
        direction = .down
      }
