...
public class NumberScrollCounter: UIView {
  ...
  /**
   ...
   - seperatorSpacing: The spacing to use between the seperator and the adjacent digits.  Defaults to `0`.
   ...
   - gradientStop: The stopping point for the gradient, where the bottom stopping point is (1 - gradientStop).
     If gradientStop is not less than 0.5 than it is ignored. If this is `nil`, then no gradient is applied.
   */
  public init(
    value: Double,
    ...
    self.splitSignPosition = splitSignPosition
  ...
  public func setValue(
    ...
    if decimalPlaces == 0, digitString.contains(seperator) {
      digitString = getDigitStringWithoutSeperator(digitString: digitString)
    }
  ...
  func updatePrefix() {
    ...
    if let prefixView = prefixView {
      ...
      animator.addAnimations { [unowned self] in
        ...
        prefixView.alpha = 1
        prefixView.textColor = self.textColor
      }
    }
  }
  ...
  func updateSuffix() {
    ...
    if let suffixView = suffixView, let scroller = digitScrollers.first {
      ...
      splitSignViews?.forEach { view in
        suffixX += view.frame.width
      }
      ...
      animator.addAnimations { [unowned self] in
        ...
        suffixView.alpha = 1
        suffixView.textColor = self.textColor
      }
    }
  }
  ...
  private var prefixView: UILabel?
  /// The view that holds the suffix, or `nil` if there is no suffix.
  private var suffixView: UILabel?
  /// The view that holds the seperator, or `nil` if there is no seperator.
  private var seperatorView: UILabel?
  /// The view that holds the negative sign, or `nil` if the number is not negative.
  private var negativeSignView: UILabel?

  /// The ',' sign between digits
  private var splitSignViews: [UILabel]?

  private let splitSignPosition: Int?
  private var splitSignViewsLocations: [Int]?
  ...
  private func getDigitStringWithoutSeperator(digitString: [String]) -> [String] {
    var digitStringWighoutSeperator = digitString
    while let lastElement = digitStringWighoutSeperator.popLast(), lastElement != seperator {
      continue
    }

    return digitStringWighoutSeperator
  }
   ...
   */
  private func getStringArray(fromValue value: Double) -> [String] {
    String(format: "%.\(decimalPlaces)f", value).compactMap { character -> String in
  ...
  private func createSplitViewsIfNeeded() {
    splitSignViewsLocations = nil

    guard let splitSignPosition = splitSignPosition else {
      return
    }
    ...
    let numberOfSplitViews: Int = max(digitString.count - 1, 0) / splitSignPosition
    let offset = digitString.count - splitSignPosition * numberOfSplitViews

    for i in 0 ..< numberOfSplitViews {
      if splitSignViewsLocations == nil {
        splitSignViewsLocations = []
      }

      let location = offset + splitSignPosition * i
      splitSignViewsLocations?.append(location)

      let count = splitSignViews?.count ?? 0
      if count <= i {
        let splitSignView = UILabel()
        splitSignView.text = ","
        splitSignView.textColor = textColor
        splitSignView.font = font
        splitSignView.sizeToFit()
        splitSignView.frame.size.width += 2 * seperatorSpacing
        splitSignView.textAlignment = .center
        splitSignView.frame.origin = CGPoint.zero
        splitSignView.alpha = 0

        if splitSignViews == nil {
          splitSignViews = []
        }

        splitSignViews?.append(splitSignView)
        addSubview(splitSignView)
      }
    }

    let count = splitSignViews?.count ?? 0
    if count > numberOfSplitViews {
      for _ in 0 ..< (count - numberOfSplitViews) {
        if let lastView = splitSignViews?.last {
          lastView.removeFromSuperview()
          splitSignViews?.removeLast()
        }
      }
    }
  }
  ...
  private func createSeperatorViewIfNeeded() {
    guard decimalPlaces > 0, seperatorView == nil else {
      animator?.addAnimations { [unowned self] in
        self.seperatorView?.textColor = self.textColor
      }
      return
    }
  ...
  private func updateDigitScrollersLayout() {
    ...
    for (index, scroller) in digitScrollers.enumerated() {
      ...
      if let splitSignViewsLocations = splitSignViewsLocations {
        let locations = splitSignViewsLocations.filter { index >= $0 }
        for i in 0 ..< locations.count {
          if let splitSignView = splitSignViews?[i] {
            x += splitSignView.frame.width
          }
        }
      }
      ...
      animator.addAnimations { [unowned self] in
        ...
        scroller.textColor = self.textColor
      }

      if index == seperatorLocation, let seperatorView = seperatorView {
        animator.addAnimations { [unowned self] in
          seperatorView.alpha = 1
          seperatorView.frame.origin.x = x - seperatorView.frame.width
          seperatorView.textColor = self.textColor
        }
      }

      if let locationIndex = splitSignViewsLocations?.firstIndex(of: index), let splitSignView = splitSignViews?[locationIndex] {
        animator.addAnimations { [unowned self] in
          splitSignView.alpha = 1
          splitSignView.frame.origin.x = x - splitSignView.frame.width
          splitSignView.textColor = self.textColor
        }
      }
    }
  }
    ...
    if includeNegativeSign {
      if let negativeSignView = negativeSignView, negativeSignView.alpha != 1 {
        animator.addAnimations { [weak self] in
          ...
          negativeSignView.textColor = self.textColor
        }
      ...
      if let negativeSignView = negativeSignView {
        negativeSignView.textColor = textColor
        animator.addAnimations {
  ...
  private func updateScrollers(add count: Int) {
    ...
    for _ in 0 ..< count {
      let digitScrollCounter = DigitScrollCounter(
        font: font,
        textColor: textColor,
        backgroundColor: digitScrollerBackgroundColor,
  ...
  /**
   ...
   - parameters:
   - digits: The digits that each item in `digitScrollers` should be scrolled to.
   - animated: Whether or not the scrolling should be animated.
   */
